...
class Banka:
    ...
    def get_hesaplar(self):
        """Hesap listesini dondurur."""
        return self.__hesaplar

    def istatistik_yaz(self, yaz_func):
        """Istatistikleri verilen fonksiyonla yazar."""
        toplam = len(self.__hesaplar)
        toplam_bakiye = sum(h.get_bakiye() for h in self.__hesaplar)
        tasarruf = sum(1 for h in self.__hesaplar if isinstance(h, TasarrufHesabi))
        yaz_func(f"Toplam Hesap    : {toplam}")
        yaz_func(f"Tasarruf Hesabi : {tasarruf}")
        yaz_func(f"Normal Hesap    : {toplam - tasarruf}")
        yaz_func(f"Toplam Bakiye   : {toplam_bakiye} TL")
